  ...
  <div class="ar">
    ...
    <div class="ar__body">
      ...
      >
        ...
        <div class="ar__goods-search">
          <input
            v-model="goodsKeyword"
            class="ar__goods-search-input"
            type="search"
      ...
      >
        ...
        <div v-if="selectedGoods" class="ar__config-scroll">
          ...
          <div class="ar__tab-group">
            ...
            <button
              class="ar__tab-btn"
              :class="{ 'ar__tab-btn--active': rightTab === 'rules' }"
              @click="rightTab = 'rules'"
            >
              <IconSparkle />
              关键词规则
            </button>
            ...
            <div class="ar__config-section">
              ...
              <div v-show="fixedMaterialExpanded" class="ar__config-section-body">
                <div class="ar__toggle-hint" style="margin-bottom: 8px;">
                  当前商品专属补充资料，会与全局模板一起发送给AI
                </div>

                <textarea
                  v-model="fixedMaterial"
                  class="ar__textarea"
                  placeholder="只填写该商品专属差异，如规格、成色、库存、注意事项等"
                  maxlength="5000"
                ></textarea>
          ...
          <!-- ====== 关键词规则视图 ====== -->
          <template v-if="rightTab === 'rules'">
            <div class="ar__rules-section">
              <div class="ar__rules-header">
                <div>
                  <div class="ar__data-section-title">关键词规则</div>
                  <div class="ar__toggle-hint">优先级：当前商品规则 > 通用规则 > 默认回复 > AI回复</div>
                </div>
                <div class="ar__rules-actions">
                  <button class="btn btn--primary" @click="openCreateRule('goods')">
                    <IconCheck />
                    当前商品
                  </button>
                  <button class="btn btn--secondary" @click="openCreateRule('global')">
                    <IconSparkle />
                    通用规则
                  </button>
                  <button class="btn btn--secondary" @click="triggerRuleImport">
                    导入CSV
                  </button>
                  <button class="btn btn--secondary" :disabled="replyRules.length === 0" @click="exportRulesCsv">
                    导出CSV
                  </button>
                  <input
                    ref="ruleImportRef"
                    class="ar__hidden-file"
                    type="file"
                    accept=".csv,text/csv"
                    @change="importRulesCsv"
                  />
                </div>
              </div>

              <div v-if="rulesLoading" class="ar__loading">
                <div class="ar__spinner"></div>
                <span>加载中...</span>
              </div>

              <div v-else-if="replyRules.length === 0" class="ar__data-empty">
                <span class="ar__data-empty-text">暂无规则</span>
              </div>

              <div v-else class="ar__rule-list">
                <div v-for="rule in replyRules" :key="rule.id" class="ar__rule-card">
                  <div class="ar__rule-card-head">
                    <div class="ar__rule-title-wrap">
                      <span class="ar__rule-title">{{ rule.ruleName || '未命名规则' }}</span>
                      <span class="ar__rule-chip">{{ getRuleScopeText(rule) }}</span>
                      <span v-if="rule.isDefault === 1" class="ar__rule-chip ar__rule-chip--default">默认</span>
                      <span
                        class="ar__rule-chip"
                        :class="rule.enabled === 1 ? 'ar__rule-chip--enabled' : 'ar__rule-chip--disabled'"
                      >
                        {{ rule.enabled === 1 ? '启用' : '停用' }}
                      </span>
                    </div>
                    <div class="ar__rule-card-actions">
                      <button class="btn btn--ghost btn--sm" @click="openEditRule(rule)">编辑</button>
                      <button class="ar__data-del-btn" @click="deleteRule(rule)">删除</button>
                    </div>
                  </div>

                  <div class="ar__rule-meta">
                    <span>{{ getRuleMatchTypeText(rule.matchType) }}</span>
                    <span>{{ getRuleReplyTypeText(rule.replyType) }}</span>
                    <span>优先级 {{ rule.priority }}</span>
                  </div>

                  <div v-if="rule.isDefault !== 1" class="ar__rule-content">
                    <span class="ar__rule-label">关键词</span>
                    <span class="ar__rule-text">{{ rule.keywords || '—' }}</span>
                  </div>
                  <div class="ar__rule-content">
                    <span class="ar__rule-label">回复</span>
                    <span class="ar__rule-text">{{ rule.replyContent || rule.imageUrls || '—' }}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      >
        <div class="ar__detail-dialog">
          ...
          <div class="ar__detail-dialog-body" v-if="recordDetail">
            ...
            <div class="ar__detail-row">
              <span class="ar__detail-label">回复类型</span>
              <span class="ar__detail-value">{{ recordDetail.replyType === 1 ? '关键词规则' : 'AI智能回复' }}</span>
            </div>
            <div v-if="recordDetail.matchedKeyword" class="ar__detail-row">
              <span class="ar__detail-label">命中关键词</span>
              <span class="ar__detail-value">{{ recordDetail.matchedKeyword }}</span>
            </div>
              ...
              <template v-if="parseTriggerContext(recordDetail.triggerContext)?.triggerMessages?.length">
                ...
                >
                  <div class="ar__detail-msg-meta">
                    ...
                    <span v-if="msg.messageTime" class="ar__detail-msg-time">{{ formatTime(msg.messageTime) }}</span>
                  </div>
            ...
            <!-- Reply content -->
            <div class="ar__detail-section">
              <div class="ar__detail-section-title">回复内容</div>
    ...
    <Transition name="overlay-fade">
      <div
        ...
        class="ar__dialog-overlay"
        @click.self="ruleDialogVisible = false"
      >
        <div class="ar__rule-dialog">
          <div class="ar__detail-dialog-header">
            <h3 class="ar__detail-dialog-title">{{ ruleForm.id ? '编辑关键词规则' : '新增关键词规则' }}</h3>
            <button class="ar__detail-dialog-close" @click="ruleDialogVisible = false">&times;</button>
          </div>
          <div class="ar__rule-dialog-body">
            <label class="ar__field">
              <span class="ar__field-label">适用范围</span>
              <select v-model="ruleForm.xyGoodsId" class="ar__field-control">
                <option :value="selectedGoods?.item.xyGoodId">当前商品</option>
                <option :value="null">通用</option>
              </select>
            </label>
            <label class="ar__field">
              <span class="ar__field-label">规则名</span>
              <input v-model="ruleForm.ruleName" class="ar__field-control" placeholder="如：价格咨询、规格说明" />
            </label>
            <div class="ar__field-grid">
              <label class="ar__field">
                <span class="ar__field-label">匹配方式</span>
                <select v-model.number="ruleForm.matchType" class="ar__field-control">
                  <option :value="1">任意关键词</option>
                  <option :value="2">全部关键词</option>
                </select>
              </label>
              <label class="ar__field">
                <span class="ar__field-label">回复方式</span>
                <select v-model.number="ruleForm.replyType" class="ar__field-control">
            ...
            <label class="ar__field ar__check-row">
              <input v-model.number="ruleForm.isDefault" type="checkbox" :true-value="1" :false-value="0" />
              <span>作为默认回复，关键词都未命中时使用</span>
            </label>
            <label v-if="ruleForm.isDefault !== 1" class="ar__field">
              <span class="ar__field-label">关键词</span>
              <textarea v-model="ruleForm.keywords" class="ar__textarea ar__textarea--compact" placeholder="每行一个关键词，也支持逗号分隔"></textarea>
            </label>
            <label v-if="ruleForm.replyType !== 2" class="ar__field">
              <span class="ar__field-label">回复内容</span>
              <textarea v-model="ruleForm.replyContent" class="ar__textarea ar__textarea--compact" placeholder="命中后发送的文字内容"></textarea>
            </label>
            <label v-if="ruleForm.replyType === 2 || ruleForm.replyType === 3" class="ar__field">
              <span class="ar__field-label">图片URL</span>
              <textarea v-model="ruleForm.imageUrls" class="ar__textarea ar__textarea--compact" placeholder="每行一个图片URL"></textarea>
            </label>
            <div class="ar__field-grid">
              <label class="ar__field">
                <span class="ar__field-label">优先级</span>
                <input v-model.number="ruleForm.priority" class="ar__field-control" type="number" min="1" />
              </label>
              <label class="ar__field">
                <span class="ar__field-label">启用</span>
                <select v-model.number="ruleForm.enabled" class="ar__field-control">
          ...
          <div class="ar__dialog-footer">
            <button class="ar__dialog-btn ar__dialog-btn--cancel" @click="ruleDialogVisible = false">取消</button>
            <button class="ar__dialog-btn ar__dialog-btn--primary" :disabled="ruleSaving" @click="saveRule">保存</button>
          </div>
        </div>
      </div>
    </Transition>
