...
import { getCompleteStatistics, type CompleteStatistics, type TrendDataPoint } from '@/api/statistics';
...
const completeStats = ref<CompleteStatistics | null>(null)
const deliveryChartMode = ref<'bar' | 'line'>('bar')
const aiChartMode = ref<'bar' | 'line'>('bar')

const formatShanghaiDate = (date: Date) => SHANGHAI_DATE_FORMATTER.format(date)

const getToday = () => formatShanghaiDate(new Date())
...
const dateQuick = ref<'today' | 'yesterday'>('today')
...
const loadDataPanel = async () => {
  ...
  try {
    const [statsRes, trendRes, completeRes] = await Promise.all([
      getDataPanelStats(selectedDate.value),
      getDataPanelTrend(),
      getCompleteStatistics({ days: 30, topN: 5 })
    ])
    ...
    if (completeRes && (completeRes.code === 200 || completeRes.code === 0) && completeRes.data) {
      completeStats.value = completeRes.data
    }
  } catch (e) {
...
const onDateChange = (val: string) => {
  ...
  else dateQuick.value = 'today' // custom date
  loadDataPanel()
}
...
const overallStats = computed(() => completeStats.value?.overall)
const latestTrend = computed(() => (completeStats.value?.trend || []).slice(-7))
const maxTrendOrders = computed(() => Math.max(...latestTrend.value.map(item => item.totalOrders || 0), 1))
...
const exportDataPanelCsv = () => {
  const rows = [
    ['类型', '日期', '订单数', '发货成功', '发货失败', '待发货', 'AI回复', '成功率'],
    [
      '统计',
      selectedDate.value,
      dataPanelStats.value.orderCount,
      dataPanelStats.value.deliverySuccessCount,
      dataPanelStats.value.deliveryFailCount,
      dataPanelStats.value.pendingDeliveryCount ?? 0,
      dataPanelStats.value.aiReplyCount,
      `${successRate.value}%`
    ],
    [],
    ['趋势日期', '发货成功', '发货失败', 'AI回复'],
    ...trendData.value.dates.map((date, index) => [
      date,
      trendData.value.deliverySuccess[index] ?? 0,
      trendData.value.deliveryFail[index] ?? 0,
      trendData.value.aiReplies[index] ?? 0
    ])
  ]
  downloadCsv(rows, `data-panel-${selectedDate.value}-${Date.now()}.csv`)
}
...
const getTrendHeight = (item: TrendDataPoint) => {
  return `${Math.max(8, Math.round((item.totalOrders || 0) / maxTrendOrders.value * 96))}px`
}
      ...
      <div class="dp" v-loading="dataPanelLoading">
        <div class="dp__header">
          ...
          <div class="dp__date-bar">
            <button
              class="dp__export-btn"
              :disabled="dataPanelLoading"
              @click="exportDataPanelCsv"
            >导出CSV</button>
            <button
              class="dp__quick-btn"
        ...
        <section class="order-stats" v-loading="orderStatsLoading">
          <div class="order-stats__head">
            <div>
              <h2 class="order-stats__title">订单统计</h2>
              <p class="order-stats__subtitle">订单状态、趋势、地区和商品排行</p>
            </div>
            <button class="dp__export-btn" :disabled="orderStatsLoading" @click="loadDataPanel">
              刷新统计
            </button>
          </div>

          <div class="order-stats__summary">
            <div class="order-stats__metric">
              <span class="order-stats__metric-label">总订单</span>
              <strong class="order-stats__metric-value">{{ overallStats?.totalOrders || 0 }}</strong>
            </div>
            <div class="order-stats__metric">
              <span class="order-stats__metric-label">待发货</span>
              <strong class="order-stats__metric-value">{{ overallStats?.pendingDeliveryOrders || 0 }}</strong>
            </div>
            <div class="order-stats__metric">
              <span class="order-stats__metric-label">已发货</span>
              <strong class="order-stats__metric-value">{{ overallStats?.shippedOrders || 0 }}</strong>
            </div>
            <div class="order-stats__metric">
              <span class="order-stats__metric-label">已完成</span>
              <strong class="order-stats__metric-value">{{ overallStats?.completedOrders || 0 }}</strong>
            </div>
            <div class="order-stats__metric">
              <span class="order-stats__metric-label">总金额</span>
              <strong class="order-stats__metric-value">¥{{ formatCurrency(overallStats?.totalAmountYuan) }}</strong>
            </div>
          </div>

          <div class="order-stats__grid">
            <div class="order-stats__panel">
              <div class="order-stats__panel-title">近7日趋势</div>
              <div v-if="latestTrend.length > 0" class="trend-bars">
                <div v-for="item in latestTrend" :key="item.date" class="trend-bars__item">
                  <div class="trend-bars__bar-wrap">
                    <div class="trend-bars__bar" :style="{ height: getTrendHeight(item) }"></div>
                  </div>
                  <span class="trend-bars__count">{{ item.totalOrders }}</span>
                  <span class="trend-bars__date">{{ item.date.slice(5) }}</span>
                </div>
              </div>
              <div v-else class="order-stats__empty">暂无趋势数据</div>
            </div>

            <div class="order-stats__panel">
              <div class="order-stats__panel-title">状态分布</div>
              <div v-if="completeStats?.statusDistribution?.length" class="rank-list">
                <div v-for="item in completeStats.statusDistribution" :key="item.status" class="rank-list__row">
                  <span class="rank-list__name">{{ item.statusText }}</span>
                  <span class="rank-list__value">{{ item.count }} 单 · {{ item.percentage }}%</span>
                </div>
              </div>
              <div v-else class="order-stats__empty">暂无状态数据</div>
            </div>

            <div class="order-stats__panel">
              <div class="order-stats__panel-title">地区 Top5</div>
              <div v-if="completeStats?.regionDistribution?.length" class="rank-list">
                <div v-for="item in completeStats.regionDistribution" :key="item.city" class="rank-list__row">
                  <span class="rank-list__name">{{ item.city || '未知地区' }}</span>
                  <span class="rank-list__value">{{ item.orderCount }} 单 · {{ item.percentage }}%</span>
                </div>
              </div>
              <div v-else class="order-stats__empty">暂无地区数据</div>
            </div>

            <div class="order-stats__panel">
              <div class="order-stats__panel-title">商品排行 Top5</div>
              <div v-if="completeStats?.goodsRanking?.length" class="rank-list">
                <div v-for="item in completeStats.goodsRanking" :key="item.xyGoodsId" class="rank-list__row">
                  <span class="rank-list__name">{{ item.goodsTitle || item.xyGoodsId }}</span>
                  <span class="rank-list__value">{{ item.orderCount }} 单 · ¥{{ formatCurrency(item.totalAmountYuan ?? Number(item.totalAmount) / 100) }}</span>
                </div>
              </div>
              <div v-else class="order-stats__empty">暂无商品数据</div>
            </div>
          </div>
        </section>
...
.dp__title { font-size: 24px; font-weight: 700; color: var(--app-text); margin: 0; }
.dp__subtitle { font-size: 13px; color: var(--app-text-soft); margin-top: 4px; display: block; }

.dp__date-bar { display: flex; align-items: center; gap: 6px; }
.dp__export-btn { padding: 5px 14px; border: 1px solid var(--app-border); border-radius: 8px; background: var(--app-surface-strong); color: var(--app-text); font-size: 13px; cursor: pointer; transition: all 0.2s; }
.dp__export-btn:hover:not(:disabled) { border-color: var(--app-border-strong); }
.dp__export-btn:disabled { color: #c0c4cc; cursor: not-allowed; }
.dp__quick-btn { padding: 5px 14px; border: 1px solid var(--app-border); border-radius: 8px; background: var(--app-surface-strong); color: var(--app-text-muted); font-size: 13px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 4px; }
.dp__quick-btn:hover { border-color: var(--app-border-strong); color: var(--app-text); }
.dp__quick-btn--active { background: var(--app-primary); color: var(--app-surface-strong); border-color: var(--app-primary); }
...
.dp__card { display: flex; align-items: center; gap: 14px; padding: 20px; background: var(--app-surface-strong); border-radius: 14px; border: 1px solid var(--app-border); }
...
.dp__card-value { font-size: 28px; font-weight: 700; color: var(--app-text); line-height: 1; }
.dp__card-label { font-size: 13px; color: var(--app-text-soft); margin-top: 4px; }

.order-stats { margin-bottom: 24px; padding: 20px; background: var(--app-surface-strong); border: 1px solid var(--app-border); border-radius: 14px; overflow: hidden; }
.order-stats__head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-bottom: 16px; }
.order-stats__title { margin: 0; color: var(--app-text); font-size: 18px; font-weight: 700; line-height: 1.2; }
.order-stats__subtitle { margin: 4px 0 0; color: var(--app-text-soft); font-size: 13px; line-height: 1.4; }
.order-stats__summary { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 10px; margin-bottom: 16px; }
.order-stats__metric { min-width: 0; padding: 14px; border-radius: 10px; background: var(--app-bg-muted); border: 1px solid var(--app-border); }
.order-stats__metric-label { display: block; margin-bottom: 6px; color: var(--app-text-soft); font-size: 12px; line-height: 1.2; }
.order-stats__metric-value { display: block; color: var(--app-text); font-size: 22px; line-height: 1.1; font-variant-numeric: tabular-nums; overflow-wrap: anywhere; }
.order-stats__grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
.order-stats__panel { min-width: 0; padding: 14px; border: 1px solid var(--app-border); border-radius: 10px; background: var(--app-surface-strong); }
.order-stats__panel-title { margin-bottom: 12px; color: var(--app-text); font-size: 14px; font-weight: 700; }
.order-stats__empty { min-height: 96px; display: flex; align-items: center; justify-content: center; color: var(--app-text-soft); font-size: 13px; }
.trend-bars { height: 140px; display: flex; align-items: flex-end; gap: 8px; min-width: 0; }
.trend-bars__item { flex: 1; min-width: 0; display: flex; flex-direction: column; align-items: center; gap: 4px; }
.trend-bars__bar-wrap { width: 100%; max-width: 32px; height: 96px; display: flex; align-items: flex-end; justify-content: center; background: var(--app-bg-muted); border-radius: 7px; overflow: hidden; }
.trend-bars__bar { width: 100%; min-height: 8px; background: #007aff; border-radius: 7px 7px 0 0; transition: height 0.25s ease; }
.trend-bars__count { color: var(--app-text); font-size: 12px; font-weight: 700; line-height: 1; }
.trend-bars__date { max-width: 100%; color: var(--app-text-soft); font-size: 11px; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.rank-list { display: flex; flex-direction: column; gap: 8px; }
.rank-list__row { display: flex; align-items: center; justify-content: space-between; gap: 10px; min-width: 0; padding-bottom: 8px; border-bottom: 1px solid var(--app-border); }
.rank-list__row:last-child { padding-bottom: 0; border-bottom: none; }
.rank-list__name { min-width: 0; color: var(--app-text); font-size: 13px; line-height: 1.35; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rank-list__value { flex-shrink: 0; color: var(--app-text-soft); font-size: 12px; line-height: 1.2; white-space: nowrap; }

.dp__charts { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
.dp__chart-card { padding: 24px; background: var(--app-surface-strong); border-radius: 14px; border: 1px solid var(--app-border); }
.dp__chart-title { font-size: 16px; font-weight: 600; color: var(--app-text); }
.dp__chart-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; }
.dp__chart-toggle { display: flex; gap: 2px; background: var(--app-bg-muted); border-radius: 6px; padding: 2px; }
.dp__toggle-btn { padding: 3px 10px; border: none; border-radius: 5px; background: transparent; color: var(--app-text-soft); font-size: 12px; cursor: pointer; transition: all 0.2s; }
.dp__toggle-btn--active { background: var(--app-surface-strong); color: var(--app-text); box-shadow: var(--app-shadow-soft); }
.dp__toggle-btn:hover:not(.dp__toggle-btn--active) { color: var(--app-text-muted); }
...
.dp__legend-item { display: flex; align-items: center; gap: 4px; font-size: 12px; color: var(--app-text-soft); }
...
.dp__bar-wrap { width: 56px; height: 160px; background: var(--app-bg-muted); border-radius: 8px; display: flex; align-items: flex-end; overflow: hidden; }
...
.dp__bar-label { font-size: 13px; color: var(--app-text-soft); }
.dp__bar-count { font-size: 18px; font-weight: 600; color: var(--app-text); }
.dp__chart-footer { margin-top: 16px; text-align: center; font-size: 13px; color: var(--app-text-soft); }
.dp__chart-footer strong { color: var(--app-text); }
...
.dp__ring-value { font-size: 28px; font-weight: 700; color: var(--app-text); line-height: 1; }
.dp__ring-label { font-size: 12px; color: var(--app-text-soft); margin-top: 2px; }

@media (max-width: 768px) {
  .dashboard__tabs { width: 100%; }
  .dashboard__tab { flex: 1; justify-content: center; padding: 8px 12px; font-size: 13px; }
  .dp__header { align-items: stretch; }
  .dp__date-bar { width: 100%; flex-wrap: wrap; }
  .dp__date-picker { flex: 1; min-width: 140px; }
  ...
  .order-stats { padding: 14px; border-radius: 12px; }
  .order-stats__head { flex-direction: column; align-items: stretch; }
  .order-stats__summary { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; }
  .order-stats__metric { padding: 12px; }
  .order-stats__metric-value { font-size: 20px; }
  .order-stats__grid { grid-template-columns: 1fr; }
  .rank-list__row { align-items: flex-start; }
  .rank-list__name { white-space: normal; overflow-wrap: anywhere; }
  .rank-list__value { text-align: right; }
...
@media (min-width: 769px) and (max-width: 1024px) {
  ...
  .order-stats__summary { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .dp__charts { grid-template-columns: 1fr; }
}

@media (max-width: 420px) {
  .dp__cards,
  .order-stats__summary { grid-template-columns: 1fr; }
  .dp__date-bar > * { flex: 1 1 calc(50% - 6px); }
  .dp__date-picker { flex-basis: 100%; }
}
</style>
