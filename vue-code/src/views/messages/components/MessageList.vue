...
import { isSystemEventMessage, renderMessage } from '@/views/online-messages/onlineMessageRender'
...
const renderedMessages = computed(() => {
  return props.messageList.map((message, index) => ({
    message,
...
const showContext = (msg: ChatMessage) => {
  ...
  currentGoodsName.value = goods?.item?.title || goods?.item?.xyGoodId || msg.xyGoodsId || ''
...
const getContentTypeText = (contentType: number, isUser: boolean) => {
  ...
  if (contentType === 3) return '语音消息'
  if (contentType === 7) return '商品卡片'
  if (contentType === 14) return '系统提示'
  if (contentType === 25) return '系统卡片'
  if (contentType === 26) return '交易卡片'
  return `系统消息`
}
    ...
    >
      <div class="msg-card__header">
        <span
          ...
          :style="{
            color: getContentTypeColor(item.message.contentType, item.message.senderUserId !== ''),
            background: getContentTypeBg(item.message.contentType, item.message.senderUserId !== '')
          }"
        >
          {{ getContentTypeText(item.message.contentType, true) }}
        </span>
        <span class="msg-card__time">{{ formatMessageTime(item.message.messageTime) }}</span>
      </div>

      <div class="msg-card__sender">
        <IconUser />
        <span>{{ item.message.senderUserName }}</span>
      </div>

      <div class="msg-card__content">
        <div v-if="item.rendered.kind === 'image'" class="rich-preview rich-preview--image">
          <img v-for="url in item.rendered.imageUrls.slice(0, 3)" :key="url" :src="url" alt="" loading="lazy" />
          <span>{{ item.rendered.text }}</span>
        </div>
        <div v-else-if="item.rendered.kind === 'audio'" class="rich-preview rich-preview--audio">
          <audio v-if="item.rendered.media?.url" :src="item.rendered.media.url" controls preload="none"></audio>
          <span v-else>{{ item.rendered.text }}</span>
        </div>
        <div v-else-if="item.rendered.kind === 'card' && item.rendered.card" class="rich-preview rich-preview--card">
          <img v-if="item.rendered.card.imageUrl" :src="item.rendered.card.imageUrl" alt="" loading="lazy" />
          <div>
            <strong>{{ item.rendered.card.title }}</strong>
            <span>{{ item.rendered.card.subtitle || item.rendered.card.orderId || item.rendered.card.tag }}</span>
          </div>
        </div>
        <span v-else>{{ item.rendered.text }}</span>
      </div>
      ...
      <div class="msg-card__footer">
        <span class="msg-card__id">ID: {{ item.message.id }}</span>
        <div class="msg-card__actions">
          <button
            class="msg-card__btn msg-card__btn--context"
            @click="showContext(item.message)"
          >
            <IconMessage />
            <span>上下文</span>
          </button>
          <button
            v-if="!isSystemEventMessage(item.message)"
            class="msg-card__btn msg-card__btn--reply"
            @click="emit('reply', item.message)"
          >
            <IconSend />
            <span>回复</span>
          </button>
  ...
  <div v-else class="table-container" :class="{ 'table-container--loading': loading }">
    <table class="table" v-if="messageList.length > 0">
      ...
      <tbody class="table__body">
        <tr v-for="item in renderedMessages" :key="item.message.id" class="table__tr">
          <td class="table__td table__td--center">
            <span class="msg-idx">{{ item.index + 1 }}</span>
          </td>
          <td class="table__td">
            <span
              class="type-tag"
          ...
          <td class="table__td">
            <span class="sender-text">{{ item.message.senderUserName }}</span>
          </td>
          <td class="table__td">
          ...
          <td class="table__td table__td--center">
            <span class="goods-id-text">{{ item.message.xyGoodsId || '-' }}</span>
          </td>
          <td class="table__td table__td--center">
            <span class="time-text">{{ formatMessageTime(item.message.messageTime) }}</span>
          </td>
          <td class="table__td table__td--actions">
            <div class="table__actions">
              <button class="table__action table__action--context" @click="showContext(item.message)">
                <IconMessage />
                <span>上下文</span>
              </button>
              <button v-if="!isSystemEventMessage(item.message)" class="table__action table__action--reply" @click="emit('reply', item.message)">
                <IconSend />
                <span>回复</span>
              </button>
            </div>
...
.rich-preview--image img + img {
  margin-left: -4px;
}

.rich-preview--audio audio {
  width: 220px;
  height: 32px;
}

.rich-preview--card div {
  min-width: 0;
  display: flex;
  flex-direction: column;
}

.rich-preview--card strong,
.rich-preview--card span,
.rich-preview > span {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
