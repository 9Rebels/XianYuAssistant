...
const loadContext = async (append = false) => {
  if (!props.sid || !props.xianyuAccountId) return
...
const isUserMessage = (msg: ChatMessage) => {
  return !!msg.senderUserId && msg.senderUserId !== props.currentAccountUnb && !isSystemEventMessage(msg)
}
...
const isMyMessage = (msg: ChatMessage) => {
  ...
  return !!msg.senderUserId && msg.senderUserId === props.currentAccountUnb
}

const isSystemMessage = (msg: ChatMessage) => {
  return isSystemEventMessage(msg) || (!isUserMessage(msg) && !isMyMessage(msg))
}
      ...
      <div class="context-content" v-loading="loading" ref="messageListRef" @scroll="handleScroll">
        ...
        <div v-if="messages.length > 0" class="message-list">
          <div
            ...
            class="message-item"
            :class="{
              'message-item--user': isUserMessage(item.message),
              'message-item--mine': isMyMessage(item.message),
              'message-item--system': isSystemMessage(item.message)
            }"
          >
            <template v-if="isSystemMessage(item.message)">
              <div class="system-text">{{ item.rendered.text.replace(/^\[|\]$/g, '') }}</div>
            </template>
            
            <template v-else>
              <div class="message-avatar">
                <div class="avatar" :class="{ 'avatar--user': isUserMessage(item.message) }">
                  <IconUser />
                </div>
              </div>
              ...
              <div class="message-body">
                <div class="message-header">
                  <span class="message-sender">{{ isUserMessage(item.message) ? item.message.senderUserName : '我' }}</span>
                  <span v-if="getMessageType(item.message)" class="message-type">{{ getMessageType(item.message) }}</span>
                  <span class="message-time">{{ formatTime(item.message.messageTime) }}</span>
                </div>
                <div class="message-text" :class="{ 'message-text--card': item.rendered.kind === 'card' }">
                  <div v-if="item.rendered.kind === 'image'" class="ctx-image-grid">
                    <a
                    ...
                    <span v-if="item.rendered.text && item.rendered.text !== '[图片]'">{{ item.rendered.text }}</span>
                  </div>
                  <div v-else-if="item.rendered.kind === 'audio'" class="ctx-audio">
                  ...
                  >
                    <div class="ctx-card__body">
                      ...
                      <span v-if="item.rendered.card.subtitle">{{ item.rendered.card.subtitle }}</span>
                      <small v-if="item.rendered.card.orderId">订单 {{ item.rendered.card.orderId }}</small>
                    </div>
