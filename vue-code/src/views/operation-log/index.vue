    ...
    <div class="ol__body">
      ...
      >
        ...
        <template v-if="selectedAccountId">
          ...
          <!-- Runtime Logs -->
          <div class="ol__runtime-log">
            <div class="ol__runtime-log-header">
              <div>
                <div class="ol__runtime-log-title">运行日志</div>
                <div class="ol__runtime-log-meta">
                  {{ runtimeLogFile || '未加载日志文件' }}
                  <span v-if="runtimeLogSize"> · {{ formatBytes(runtimeLogSize) }}</span>
                  <span v-if="runtimeLogUpdatedAt"> · {{ formatTime(runtimeLogUpdatedAt) }}</span>
                </div>
              </div>
              <div class="ol__runtime-log-actions">
                <div class="ol__runtime-log-select-wrap">
                  <select
                    v-model="selectedRuntimeLogFile"
                    class="ol__runtime-log-select"
                    :disabled="runtimeLogLoading || runtimeLogFiles.length === 0"
                    @change="handleRuntimeLogFileChange"
                  >
                    <option
                      v-for="file in runtimeLogFiles"
                      :key="file.file"
                      :value="file.file"
                    >
                  ...
                  <span class="ol__runtime-log-select-icon">
                    <IconChevronDown />
                  </span>
                </div>
                <button
                  class="btn btn--secondary btn--sm"
                  :class="{ 'btn--loading': runtimeLogLoading }"
                ...
                <button
                  ...
                  :class="{ 'btn--loading': runtimeLogLoading }"
                  :disabled="runtimeLogLoading"
                  @click="showRecentRuntimeLog"
                >
                  最近200行
                </button>
                <button
                  class="btn btn--danger btn--sm"
                  :disabled="runtimeLogLoading || !runtimeLogFile"
                  @click="handleClearRuntimeLog"
                >
                  清空日志
                </button>
              </div>
            </div>
            <pre class="ol__runtime-log-box">{{ runtimeLogLines.join('\n') || (runtimeLogLoading ? '日志加载中...' : '暂无运行日志') }}</pre>
          </div>
        </template>
      </div>
    </div>
