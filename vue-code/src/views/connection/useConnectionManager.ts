...
export function useConnectionManager() {
  ...
  const resolveManualVerificationIfHealthy = (status?: ConnectionStatus | null) => {
    ...
    for (const account of accounts.value) {
      try {
        ...
        if (response.code === 0 || response.code === 200) {
          ...
          resolveManualVerificationIfHealthy(status)
        }
      } catch {
        // 单个账号查询失败不影响其他账号
      }
  ...
  const loadConnectionStatus = async (accountId: number, silent = false) => {
    ...
    try {
      ...
      if (response.code === 0 || response.code === 200) {
        ...
        resolveManualVerificationIfHealthy(connectionStatus.value)
  ...
  const addLog = (message: string, isError = false) => {
    const time = formatTime(Date.now()).split(' ')[1] || formatTime(Date.now())
