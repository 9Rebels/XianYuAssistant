  ...
  <div v-if="isMobile" class="card-list" :class="{ 'card-list--loading': loading }">
    ...
    >
      ...
      <div class="order-card__body">
        ...
        <div v-if="order.content" class="order-card__row order-card__row--stack">
          <span class="order-card__label">内容</span>
          <button
            class="record-expand-text order-card__value order-card__value--long"
            :class="{ 'record-expand-text--expanded': isRecordExpanded(order.id) }"
            type="button"
            @click="toggleRecordExpanded(order.id)"
          >
            {{ order.content }}
          </button>
        </div>
        <div v-if="order.state === -1 && order.failReason" class="order-card__row order-card__row--stack">
          <span class="order-card__label">原因</span>
          <button
            class="record-expand-text order-card__fail-reason"
            :class="{ 'record-expand-text--expanded': isRecordExpanded(order.id) }"
            type="button"
            @click="toggleRecordExpanded(order.id)"
          >
      ...
      <div class="order-card__footer">
        ...
        <button
          v-if="order.orderId && (order.confirmState || 0) !== 1"
        ...
        <span v-else-if="order.orderId" class="order-card__action-placeholder">已确认</span>
      </div>
    </div>
      ...
      <tbody class="table__body">
        <tr v-for="order in orderList" :key="order.id" class="table__tr">
          ...
          <td class="table__td">
            <button
              v-if="order.content"
              class="record-expand-text content-text"
              :class="{ 'record-expand-text--expanded': isRecordExpanded(order.id) }"
              type="button"
              @click="toggleRecordExpanded(order.id)"
            >
          ...
          <td class="table__td table__td--center">
            ...
            <button
              v-if="order.state === -1 && order.failReason"
              class="record-expand-text fail-reason"
              :class="{ 'record-expand-text--expanded': isRecordExpanded(order.id) }"
              type="button"
              @click="toggleRecordExpanded(order.id)"
            >
          ...
          <td class="table__td table__td--actions">
            ...
            <span v-else-if="order.orderId" class="table__action-placeholder">已确认</span>
...
.order-card__action-placeholder {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  flex: 1;
...
.order-card__fail-reason.record-expand-text--expanded,
.fail-reason.record-expand-text--expanded {
  max-width: 100%;
  white-space: pre-wrap;
  overflow-wrap: anywhere;
}
