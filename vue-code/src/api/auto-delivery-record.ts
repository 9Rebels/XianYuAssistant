...
export interface AutoDeliveryRecord {
  ...
  externalConfirmState?: number;
  externalReturnState?: number;
  externalReturnReason?: string;
  createTime: string;
}
...
export interface ManualReturnApiDeliveryReq {
  xianyuAccountId: number;
  xyGoodsId: string;
  recordId: number;
}

export function manualReturnApiDelivery(data: ManualReturnApiDeliveryReq) {
  return request<string>({
    url: '/autoDelivery/manualReturn',
    method: 'POST',
    data
  });
}
