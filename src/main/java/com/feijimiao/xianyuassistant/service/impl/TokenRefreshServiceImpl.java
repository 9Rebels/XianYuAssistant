...
public class TokenRefreshServiceImpl implements TokenRefreshService {
    ...
    private boolean refreshMh5tkTokenWithRetry(Long accountId, int retryCount, boolean hasLoginAttempted) {
        try {
            ...
            if (!accountIdentityGuard.canUseCookie(accountId, oldCookieStr)) {
                log.warn("【账号{}】Cookie身份不匹配，停止刷新_m_h5_tk token", accountId);
                return false;
            }
            ...
            OkHttpClient okHttpClient = accountProxyHelper.createHttpClient(cookieJar, accountId);
                ...
                if (newMh5tk != null && !newMh5tk.isEmpty()) {
                    ...
                    if (mh5tkChanged) {
                        if (!accountIdentityGuard.canUseCookie(accountId, newCookieStr)) {
                            log.warn("【账号{}】_m_h5_tk刷新返回Cookie身份不匹配，已拒绝写回", accountId);
                            return false;
                        }
    ...
    public boolean refreshWebSocketToken(Long accountId) {
        try {
            if (webSocketTokenService.isCaptchaWaiting(accountId)) {
                log.info("【账号{}】正在等待人机验证，跳过本轮WebSocket token主动刷新", accountId);
                return false;
            }
    ...
    private void triggerCookieExpireNotify(Long accountId) {
        try {
            ...
            if (lastTime != null && (now - lastTime) < COOKIE_NOTIFY_INTERVAL_MS) {
                log.debug("【账号{}】Cookie过期通知防抖中，跳过", accountId);
                return;
            }
            lastCookieExpireNotifyTimes.put(accountId, now);
            String title = "【闲鱼助手】Cookie 已过期";
            String content = notificationContentBuilder.eventContent(
                    accountId,
                    "Cookie 已确认无法自动续期",
                    "系统保活和 Token 刷新均未恢复该账号 Cookie，后续自动回复、自动发货和连接续期都会受影响。",
                    "请重新登录或手动刷新 Cookie，再检查连接状态是否恢复。"
            );
            notificationService.notifyEvent(NotificationService.EVENT_COOKIE_EXPIRE, title, content);
        } catch (Exception e) {
            log.warn("触发Cookie过期通知异常: {}", e.getMessage());
        }
    }
}
