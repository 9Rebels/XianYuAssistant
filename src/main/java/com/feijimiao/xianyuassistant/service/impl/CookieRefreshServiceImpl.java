...
public class CookieRefreshServiceImpl implements CookieRefreshService {
    ...
    private boolean doCheckLoginStatusWithRetry(Long accountId, int retryCount, boolean logOperation) {
        ...
        try {
            ...
            if (!accountIdentityGuard.canUseCookie(accountId, oldCookieStr)) {
                log.warn("【账号{}】Cookie身份不匹配，停止登录状态检查", accountId);
                return false;
            }
            ...
            OkHttpClient httpClient = accountProxyHelper.createHttpClient(cookieJar, accountId);
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                    ...
                    try {
                        ...
                        notificationService.notifyEvent(
                                ...
                                notificationContentBuilder.eventContent(
                                        accountId,
                                        "hasLogin 时触发风控验证",
                                        "系统检测到闲鱼风控，当前账号需要人工处理滑块或人机验证。",
                                        "请打开闲鱼网页版消息页完成验证，验证通过后刷新 Cookie 并重新检查连接。"
                                )
                        );
                    } catch (Exception e) {
                        log.error("【账号{}】发送风控验证通知失败", accountId, e);
                    }
                ...
                if (content != null && Boolean.TRUE.equals(content.get("success"))) {
                    ...
                    if (cookieChanged) {
                        if (!accountIdentityGuard.canUseCookie(accountId, newCookieStr)) {
                            log.warn("【账号{}】hasLogin返回Cookie身份不匹配，已拒绝写回", accountId);
                            return false;
                        }
    ...
    private boolean refreshCookieWithBrowser(Long accountId) {
        ...
        if (!accountIdentityGuard.canUseCookie(accountId, cookie.getCookieText())) {
            log.warn("【账号{}】浏览器兜底刷新前Cookie身份不匹配，停止刷新", accountId);
            markAccountAsCookieRefreshAbnormal(accountId, "浏览器兜底刷新失败：Cookie身份与当前账号不一致");
            return false;
        }
        ...
        try (Playwright playwright = Playwright.create()) {
            ...
            try (Browser browser = playwright.chromium().launch(launchOptions)) {
                ...
                try (BrowserContext context = browser.newContext(contextOptions)) {
                    ...
                    if (!accountIdentityGuard.canUseCookie(accountId, refreshedCookieText)) {
                        log.warn("【账号{}】浏览器兜底刷新返回Cookie身份不匹配，已拒绝写回", accountId);
                        markAccountAsCookieRefreshAbnormal(accountId, "浏览器兜底刷新失败：新Cookie身份不匹配");
                        return false;
                    }
