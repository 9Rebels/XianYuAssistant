...
public class ImageUploadServiceImpl implements ImageUploadService {
    ...
    public ResultObject<String> uploadImage(Long accountId, byte[] imageData, String filename) {
        try {
            ...
            if ("COOKIE_EXPIRED".equals(cdnUrl)) {
                return ResultObject.failed("已尝试自动刷新和验证，仍失败，请人工更新Cookie或完成滑块验证");
            }
            if (cdnUrl == null) {
    ...
    private String uploadToGoofishCDN(Long accountId, String cookie, byte[] imageData, String filename) {
        String cdnUrl = uploadToGoofishCDN(cookie, imageData, filename);
        if (!"COOKIE_EXPIRED".equals(cdnUrl)) {
            return cdnUrl;
        }

        CookieRecoveryResult recovery = cookieRecoveryService.recover(accountId, "图片上传", "Cookie失效/过期");
        if (recovery.isSuccess() && recovery.getCookieText() != null && !recovery.getCookieText().isBlank()) {
            return uploadToGoofishCDN(recovery.getCookieText(), imageData, filename);
        }
        log.warn("【账号{}】图片上传触发Cookie恢复失败: {}", accountId,
                recovery.getMessage());
