...
public class ItemController {
    ...
    public ResultObject<ItemListFromDbRespDTO> getItemsFromDb(@RequestBody ItemListFromDbReqDTO reqDTO) {
        try {
            log.info("从数据库获取商品列表: status={}, keyword={}, pageNum={}, pageSize={}",
                    reqDTO.getStatus(), reqDTO.getKeyword(), reqDTO.getPageNum(), reqDTO.getPageSize());
     ...
     */
    @PostMapping("/publish")
    public ResultObject<PublishItemRespDTO> publishItem(@RequestBody PublishItemReqDTO reqDTO) {
        try {
            log.info("发布商品请求: xianyuAccountId={}, title={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getTitle());
            return itemPublishService.publishItem(reqDTO);
        } catch (Exception e) {
            log.error("发布商品失败", e);
            return ResultObject.failed("发布商品失败: " + e.getMessage());
        }
    }
    ...
    public ResultObject<String> offShelfItem(@RequestBody DeleteItemReqDTO reqDTO) {
        try {
            log.info("下架商品请求: xianyuAccountId={}, xyGoodsId={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId());
            return itemService.offShelfItem(reqDTO);
        } catch (Exception e) {
            log.error("下架商品失败", e);
            return ResultObject.failed("下架商品失败: " + e.getMessage());
        }
    }

    @PostMapping("/republish")
    public ResultObject<String> republishItem(@RequestBody DeleteItemReqDTO reqDTO) {
        try {
            log.info("恢复上架商品请求: xianyuAccountId={}, xyGoodsId={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId());
            return itemService.republishItem(reqDTO);
        } catch (Exception e) {
            log.error("恢复上架商品失败", e);
            return ResultObject.failed("恢复上架商品失败: " + e.getMessage());
        }
    }

    @PostMapping("/remoteDelete")
    public ResultObject<String> remoteDeleteItem(@RequestBody DeleteItemReqDTO reqDTO) {
        try {
            log.info("远程删除闲鱼商品请求: xianyuAccountId={}, xyGoodsId={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId());
            return itemService.remoteDeleteItem(reqDTO);
        } catch (Exception e) {
            log.error("远程删除闲鱼商品失败", e);
            return ResultObject.failed("远程删除闲鱼商品失败: " + e.getMessage());
        }
    }

    @PostMapping("/updatePrice")
    public ResultObject<String> updateItemPrice(@RequestBody UpdateItemPriceReqDTO reqDTO) {
        try {
            log.info("修改商品价格请求: xianyuAccountId={}, xyGoodsId={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId());
            return itemService.updateItemPrice(reqDTO);
        } catch (Exception e) {
            log.error("修改商品价格失败", e);
            return ResultObject.failed("修改商品价格失败: " + e.getMessage());
        }
    }

    @PostMapping("/updateStock")
    public ResultObject<String> updateItemStock(@RequestBody UpdateItemStockReqDTO reqDTO) {
        try {
            log.info("修改商品库存请求: xianyuAccountId={}, xyGoodsId={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId());
            return itemService.updateItemStock(reqDTO);
        } catch (Exception e) {
            log.error("修改商品库存失败", e);
            return ResultObject.failed("修改商品库存失败: " + e.getMessage());
        }
    }
