...
public class AutoDeliveryController {
     ...
     * 手动回库 API 发货记录
     */
    @PostMapping("/manualReturn")
    public ResultObject<String> manualReturnApiDelivery(@Valid @RequestBody ManualReturnApiDeliveryReqDTO reqDTO) {
        try {
            log.info("手动回库请求: xianyuAccountId={}, xyGoodsId={}, recordId={}",
                    reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId(), reqDTO.getRecordId());
            return autoDeliveryService.manualReturnApiDelivery(reqDTO);
        } catch (Exception e) {
            log.error("手动回库失败", e);
            return ResultObject.failed("手动回库失败: " + e.getMessage());
        }
    }
}
