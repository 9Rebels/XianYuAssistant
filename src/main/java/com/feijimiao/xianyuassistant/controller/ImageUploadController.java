...
public class ImageUploadController {
    ...
    public ResultObject<String> uploadImage(
        ...
        try {
            ...
            ResultObject<String> result = imageUploadService.uploadImage(accountId, imageData, filename);
            saveToMediaLibraryIfSuccess(accountId, filename, file.getSize(), result);
            return result;
    ...
    private void saveToMediaLibraryIfSuccess(Long accountId, String filename, Long fileSize, ResultObject<String> result) {
        if (result == null || result.getCode() == null || result.getCode() != 200 || result.getData() == null) {
            return;
        }
        try {
            XianyuMediaLibrary media = new XianyuMediaLibrary();
            media.setXianyuAccountId(accountId);
            media.setFileName(filename);
            media.setFileSize(fileSize);
            media.setMediaUrl(result.getData());
            media.setCreatedTime(DateTimeUtils.currentShanghaiTime());
            mediaLibraryMapper.insert(media);
            log.info("媒体库素材已保存: accountId={}, mediaId={}", accountId, media.getId());
        } catch (Exception e) {
            log.warn("图片已上传成功，但写入媒体库失败: accountId={}, url={}", accountId, result.getData(), e);
        }
    }
}
