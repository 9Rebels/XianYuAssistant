...
public class WebSocketController {
    ...
    public ResultObject<String> sendMessage(@RequestBody SendMessageReqDTO reqDTO) {
        try {
            ...
            if (!isSendGoodsOwnedByAccount(reqDTO.getXianyuAccountId(), reqDTO.getXyGoodsId())) {
                return ResultObject.failed("当前会话商品不属于该账号，已拒绝发送以避免串号");
            }
    ...
    public ResultObject<WebSocketStatusRespDTO> getWebSocketStatus(@RequestBody GetWebSocketStatusReqDTO reqDTO) {
        try {
            ...
            respDTO.setNeedCaptcha(tokenService.isCaptchaWaiting(reqDTO.getXianyuAccountId()));
            respDTO.setCaptchaUrl(tokenService.getCaptchaUrl(reqDTO.getXianyuAccountId()));
    ...
    private boolean isSendGoodsOwnedByAccount(Long accountId, String xyGoodsId) {
        if (xyGoodsId == null || xyGoodsId.isBlank()) {
            return true;
        }
        try {
            return goodsInfoService.getByXyGoodIdAndAccountId(accountId, xyGoodsId) != null;
        } catch (Exception e) {
            log.warn("发送前校验商品归属失败: accountId={}, xyGoodsId={}", accountId, xyGoodsId, e);
            return false;
        }
    }
     ...
     * 手动重试自动滑块
     */
    @PostMapping("/retryAutoCaptcha")
    public ResultObject<String> retryAutoCaptcha(@RequestBody RetryAutoCaptchaReqDTO reqDTO) {
        try {
            log.info("手动重试自动滑块: xianyuAccountId={}", reqDTO.getXianyuAccountId());
            ...
            com.feijimiao.xianyuassistant.service.WebSocketTokenService tokenService =
                    applicationContext.getBean(com.feijimiao.xianyuassistant.service.WebSocketTokenService.class);
            boolean success = tokenService.retryAutoCaptcha(reqDTO.getXianyuAccountId());
            ...
            operationLogService.log(reqDTO.getXianyuAccountId(),
                    ...
                    null,
                    null,
                    success ? null : com.feijimiao.xianyuassistant.service.CaptchaService.AUTO_SLIDER_FAILED_MESSAGE,
                    null);

            if (success) {
                return ResultObject.success("自动滑块成功，Cookie 已更新");
            }
            return ResultObject.failed(com.feijimiao.xianyuassistant.service.CaptchaService.AUTO_SLIDER_FAILED_MESSAGE);
        } catch (Exception e) {
            log.error("手动重试自动滑块失败", e);
            return ResultObject.failed("手动重试自动滑块失败: " + e.getMessage());
        }
    }
    ...
    public ResultObject<String> passwordLogin(@RequestBody PasswordLoginReqDTO reqDTO) {
        Long accountId = reqDTO == null ? null : reqDTO.getXianyuAccountId();
        try {
            log.info("手动触发账号密码登录: xianyuAccountId={}", accountId);
            ...
            String cookieText = passwordLoginService.tryPasswordLogin(accountId);
            if (cookieText == null || cookieText.trim().isEmpty()) {
                logPasswordLogin(accountId, false, "账号密码登录失败，请按前端弹窗完成人脸/扫码/二维码验证");
                return ResultObject.failed("账号密码登录失败，请按前端弹窗完成人脸/扫码/二维码验证");
            }

            String unb = extractUnbFromCookie(cookieText);
            if (unb == null || unb.isEmpty()) {
                logPasswordLogin(accountId, false, "账号密码登录返回的Cookie缺少UNB");
                return ResultObject.failed("账号密码登录返回的Cookie缺少UNB，已拒绝更新");
            }
            ...
            if (!updated) {
                logPasswordLogin(accountId, false, "Cookie身份与当前账号不一致");
            ...
            return ResultObject.success("账号密码登录成功，Cookie 已更新");
        } catch (Exception e) {
            log.error("手动触发账号密码登录失败: xianyuAccountId={}", accountId, e);
     ...
     */
    @Data
    public static class RetryAutoCaptchaReqDTO {
        private Long xianyuAccountId;  // 账号ID
    }
     ...
     */
    @Data
    public static class PasswordLoginReqDTO {
        private Long xianyuAccountId;  // 账号ID
    }
    ...
    public static class WebSocketStatusRespDTO {
        ...
        private Boolean needCaptcha;   // 是否正在等待滑块验证
