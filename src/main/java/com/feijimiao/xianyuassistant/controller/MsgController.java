...
public class MsgController {
    ...
    public ResultObject<?> getOnlineConversations(@RequestParam Long xianyuAccountId,
                                                  @RequestParam(required = false) Integer limit) {
        try {
            log.info("查询在线消息会话列表: xianyuAccountId={}, limit={}", xianyuAccountId, limit);
            return chatMessageService.getOnlineConversations(xianyuAccountId, limit);
        } catch (Exception e) {
            log.error("查询在线消息会话列表失败", e);
            return ResultObject.failed("查询在线消息会话列表失败: " + e.getMessage());
        }
    }
}
