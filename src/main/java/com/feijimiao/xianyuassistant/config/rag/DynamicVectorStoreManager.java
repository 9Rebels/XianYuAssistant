...
public class DynamicVectorStoreManager {
    ...
    public VectorStore getVectorStore() {
        ...
        // 如果 Embedding 专用 API Key 未配置，使用激活提供商的 API Key
        if (embeddingApiKey == null || embeddingApiKey.trim().isEmpty()) {
            ...
            embeddingApiKey = activeConfig.getApiKey();
        }

        // 如果 Embedding 专用 Base URL 未配置，使用激活提供商的 Base URL
        if (embeddingBaseUrl == null || embeddingBaseUrl.trim().isEmpty()) {
            DynamicAIChatClientManager.AIProviderConfig activeConfig = dynamicAIChatClientManager.resolveActiveProviderConfig();
            embeddingBaseUrl = activeConfig.getBaseUrl();
        }
