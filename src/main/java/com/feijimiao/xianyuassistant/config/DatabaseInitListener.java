...
public class DatabaseInitListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            // 4. 创建依赖新增字段的兼容索引
            checkAndCreateCompatibilityIndexes(stmt);

            // 4.1 补充鱼小铺字段的兼容迁移
            ensureFishShopColumns(stmt);
    ...
    private void checkAndCreateTriggers(Statement stmt, SqlSchemaParser.SchemaDefinition schema) throws Exception {
        ...
        for (Map.Entry<String, SqlSchemaParser.TriggerDefinition> entry : schema.getTriggers().entrySet()) {
            ...
            if (!existingTriggers.contains(triggerName)) {
                ...
                createdCount++;
            } else if (isTriggerDefinitionChanged(stmt, triggerName, trigger.getCreateSql())) {
                log.info("  🔄 更新触发器: {}", triggerName);
        ...
        if (updatedCount > 0) {
            log.info("✅ 更新了 {} 个触发器", updatedCount);
        }
    ...
    private boolean isTriggerDefinitionChanged(Statement stmt, String triggerName, String expectedSql) throws Exception {
        ResultSet rs = stmt.executeQuery(
            "SELECT sql FROM sqlite_master WHERE type='trigger' AND name='" + triggerName + "'"
        );
        String existingSql = rs.next() ? rs.getString("sql") : null;
        rs.close();
        return !normalizeSql(existingSql).equals(normalizeSql(expectedSql));
    }
    ...
    private void checkAndCreateCompatibilityIndexes(Statement stmt) throws Exception {
        if (tableExists(stmt, "xianyu_goods") && getTableColumns(stmt, "xianyu_goods").contains("sort_order")) {
            stmt.execute("CREATE INDEX IF NOT EXISTS idx_goods_account_sort ON xianyu_goods(xianyu_account_id, sort_order)");
        }
        if (tableExists(stmt, "xianyu_goods_auto_delivery_config")) {
            stmt.execute("DROP INDEX IF EXISTS idx_auto_delivery_config_unique");
            stmt.execute("CREATE INDEX IF NOT EXISTS idx_auto_delivery_config_account_goods ON xianyu_goods_auto_delivery_config(xianyu_account_id, xy_goods_id)");
        }
    }

    private void ensureFishShopColumns(Statement stmt) throws Exception {
        if (!tableExists(stmt, "xianyu_account")) {
            return;
        }
        Set<String> columns = getTableColumns(stmt, "xianyu_account");
        if (!columns.contains("fish_shop_user")) {
            stmt.execute("ALTER TABLE xianyu_account ADD COLUMN fish_shop_user TINYINT DEFAULT 0");
        }
        if (!columns.contains("fish_shop_level")) {
            stmt.execute("ALTER TABLE xianyu_account ADD COLUMN fish_shop_level VARCHAR(50)");
        }
        if (!columns.contains("fish_shop_score")) {
            stmt.execute("ALTER TABLE xianyu_account ADD COLUMN fish_shop_score INTEGER");
        }
    }

    private void backfillGoodsSortOrder(Statement stmt) throws Exception {
        if (!tableExists(stmt, "xianyu_goods") || !getTableColumns(stmt, "xianyu_goods").contains("sort_order")) {
            return;
        }

        ResultSet rs = stmt.executeQuery(
                "SELECT COUNT(*) FROM xianyu_goods WHERE sort_order IS NULL OR sort_order = 2147483647"
        );
        ...
        String triggerSql = getTriggerSql(stmt, "update_xianyu_goods_time");
        if (triggerSql != null) {
            stmt.execute("DROP TRIGGER IF EXISTS update_xianyu_goods_time");
        }
        try {
            stmt.executeUpdate(
                    "UPDATE xianyu_goods " +
                            ...
                            "PARTITION BY xianyu_account_id " +
                            "ORDER BY updated_time ASC, created_time ASC, id ASC" +
                            ") rn FROM xianyu_goods" +
                            ") ranked WHERE ranked.id = xianyu_goods.id" +
                            ") WHERE sort_order IS NULL OR sort_order = 2147483647"
            );
        } finally {
            if (triggerSql != null) {
        ...
        log.info("✅ 已回填历史商品客户端排序: {} 条", pendingCount);
    }

    private String getTriggerSql(Statement stmt, String triggerName) throws Exception {
        ResultSet rs = stmt.executeQuery(
                "SELECT sql FROM sqlite_master WHERE type='trigger' AND name='" + triggerName + "'"
        );
