...
public interface XianyuChatMessageMapper {
     ...
     * 根据消息ID查询。
     */
    @Select("SELECT * FROM xianyu_chat_message WHERE id = #{id}")
    XianyuChatMessage findById(@Param("id") Long id);
    ...
    @Select("SELECT * FROM xianyu_chat_message " +
            ...
            "ORDER BY message_time DESC " +
            "LIMIT #{limit}")
    List<XianyuChatMessage> findRecentForConversations(@Param("accountId") Long accountId,
                                                       @Param("limit") int limit);
    ...
    @Select("<script>" +
            ...
            "AND (content_type IS NULL OR content_type NOT IN (14, 25)) " +
            "</if>" +
            "ORDER BY message_time DESC " +
            "LIMIT #{limit} OFFSET #{offset}" +
            "</script>")
    ...
    @Select("SELECT * FROM xianyu_chat_message " +
            ...
            "ORDER BY message_time DESC " +
            "LIMIT #{limit} OFFSET #{offset}")
    List<XianyuChatMessage> findRecentByAccountIdAndSId(@Param("accountId") Long accountId,
