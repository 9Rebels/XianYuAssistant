...
public class SyncMessageHandler extends AbstractLwpHandler {
    ...
    private void parseAndPublishEvent(String accountId, String decryptedData, String lwp) {
        try {
            ...
            if (typeObj != null && "2".equals(typeObj.toString())) {
                readStateService.markReadReceipt(Long.parseLong(accountId), decryptedData);
                return;
            }
            ...
            if (field10 instanceof Map) {
                ...
                receiverUserId = extractString(field10Map, "receiver");
            ...
            if (!isMessageForAccount(accountIdLong, message.getXyGoodsId(), receiverUserId, message.getSenderUserId())) {
                log.info("【账号{}】消息归属不匹配当前账号，跳过: pnmId={}, xyGoodsId={}, receiver={}, senderUserId={}",
                        accountId, message.getPnmId(), message.getXyGoodsId(), receiverUserId, message.getSenderUserId());
                return;
            }
     ...
     * 从Map中提取字符串值
     */
    private boolean isMessageForAccount(Long accountId, String xyGoodsId, String receiverUserId, String senderUserId) {
        try {
            boolean hasGoodsId = hasText(xyGoodsId);
            if (hasGoodsId) {
                if (isGoodsOwnedByAccount(accountId, xyGoodsId)) {
                    return true;
                }
                if (isGoodsKnownByOtherAccount(accountId, xyGoodsId)) {
                    return false;
                }
            }

            XianyuAccount account = accountMapper != null ? accountMapper.selectById(accountId) : null;
            String ownUserId = account != null ? normalizeUserId(account.getUnb()) : "";
            if (!hasText(ownUserId)) {
                return false;
            }
            String receiver = normalizeUserId(receiverUserId);
            if (hasText(receiver)) {
                return ownUserId.equals(receiver);
            }

            String sender = normalizeUserId(senderUserId);
            if (ownUserId.equals(sender)) {
                return true;
            }

            // 闲鱼的系统提醒/活动提醒经常没有 receiver，senderUserId 是对方或系统账号。
            // 这类消息已经来自当前账号的 WebSocket 连接，缺少明确反证时应归入当前账号。
            return true;
        } catch (Exception e) {
            log.warn("【账号{}】校验消息归属失败，跳过消息: {}", accountId, e.getMessage());
            return false;
        }
    }
    ...
    private boolean isGoodsOwnedByAccount(Long accountId, String xyGoodsId) {
        try {
            return goodsInfoService != null
                    && goodsInfoService.getByXyGoodIdAndAccountId(accountId, xyGoodsId) != null;
        } catch (Exception e) {
            log.warn("【账号{}】查询当前账号商品归属失败: xyGoodsId={}, error={}",
                    accountId, xyGoodsId, e.getMessage());
            return false;
        }
    }

    private boolean isGoodsKnownByOtherAccount(Long accountId, String xyGoodsId) {
        try {
            if (goodsInfoService == null) {
                return false;
            }
            XianyuGoodsInfo goods = goodsInfoService.getByXyGoodId(xyGoodsId);
            return goods != null
                    && goods.getXianyuAccountId() != null
                    && !goods.getXianyuAccountId().equals(accountId);
        } catch (Exception e) {
            log.warn("【账号{}】查询其他账号商品归属失败: xyGoodsId={}, error={}",
                    accountId, xyGoodsId, e.getMessage());
            return false;
        }
    }
